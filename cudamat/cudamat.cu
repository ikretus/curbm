...
extern int apply_relu(cudamat* mat, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
    ...
    kApplyReLU<<<NUM_VECTOR_OP_BLOCKS,NUM_VECTOR_OP_THREADS_PER_BLOCK>>>(mat->data_device, target->data_device, len);
