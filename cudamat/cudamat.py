...
_cudamat.mult_by_sigmoid_deriv.restype = ct.c_int
...
class CUDAMatrix(object):
    ...
    def apply_cos(self, target = None):
        """
        Apply cos sigmoid to each element of the matrix.
        """
        return cos(self, target)

    def apply_sin(self, target = None):
        """
        Apply sin sigmoid to each element of the matrix.
        """
        return sin(self, target)

    def apply_sigmoid(self, target = None):
        """
        Apply logistic sigmoid to each element of the matrix.
        """
        return sigmoid(self, target)

    def apply_softplus(self, target = None):
        """
        Apply softplus to each element of the matrix.
        """
        return softplus(self, target)

    def apply_relu(self, target = None):
        """
        Apply relu to each element of the matrix.
        """
        return relu(self, target)
...
def relu(mat, target = None):
    """
    Apply relu(x) = max(0, x) to each element of the matrix mat.
    """
    if not target:
        target = mat

    err_code = _cudamat.apply_relu(mat.p_mat, target.p_mat)
    if err_code:
        raise generate_exception(err_code)
